...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/prima-materia";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/prima-materia-hidden-route-20260523",
);
...
async function captureViewport(browser, viewport) {
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/audio/perfect-conductors/prima-materia",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/prima-materia-lux-speaker",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "prima-materia-proof-qa-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      "# MNT-SITE-VIS-021A Prima Materia Hidden Route QA",
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Prima Materia line and request paths.",
