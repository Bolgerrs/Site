...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/exhibition-display";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/exhibition-display-hidden-route-20260523",
);
...
async function captureViewport(browser, viewport) {
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/exhibition-displays",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/exhibition-wall",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "exhibition-display-proof-qa-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      "# MNT-SITE-VIS-021A Exhibition Display Hidden Route QA",
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Exhibition Display direction and product request paths.",
