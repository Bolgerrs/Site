...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/living-glass-gostiny-dvor";
const qaLabel = process.env.MONTELAR_QA_LABEL || "Living Glass Gostiny Dvor";
const reportSlug = process.env.MONTELAR_QA_REPORT_SLUG || "living-glass-gostiny-proof";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/living-glass-gostiny-dvor-hidden-route-20260523",
);
  ...
  return page.evaluate((scanLabel) => {
    ...
    for (const element of document.querySelectorAll(selectors.join(","))) {
      ...
      if (ownOverflow || viewportOverflow) {
        issues.push({
          label: scanLabel,
          className: element.className?.toString?.() || element.tagName.toLowerCase(),
          text,
          rect: {
...
async function scrollToProofSection(page, selector) {
  await page.evaluate((targetSelector) => {
    ...
    const headerHeight = Number.parseFloat(
      window.getComputedStyle(root).getPropertyValue("--shell-header-height"),
    ) || 84;
...
async function captureViewport(browser, viewport) {
  const context = await browser.newContext({
    ...
    recordVideo: viewport.video
      ? { dir: path.join(artifactRoot, "videos"), size: { width: viewport.width, height: viewport.height } }
      : undefined,
  });
  ...
  const frameObservation = await page.evaluate(async () => {
    ...
    return new Promise((resolve) => {
      function tick(now) {
        ...
        if (frames === 8) {
          window.scrollTo({ top: Math.round(document.body.scrollHeight * 0.62), behavior: "smooth" });
        }
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/invisible-display",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/living-glass-oled",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, `${reportSlug}-qa-report.json`), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      `# MNT-SITE-VIS-021A ${qaLabel} Hidden Route QA`,
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Living Glass direction and request paths.",
