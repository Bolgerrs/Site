...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/pictorial-art-display";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/pictorial-art-display-hidden-route-20260523",
);
...
async function scanLayout(page, label) {
  return page.evaluate((scanLabel) => {
    ...
    for (const element of document.querySelectorAll(selectors.join(","))) {
      ...
      if (element.scrollWidth > element.clientWidth + 2 || rect.left < -1 || rect.right > viewport.width + 1) {
        issues.push({
          label: scanLabel,
...
async function captureViewport(browser, viewport) {
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/pictorial-art-display",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/pictorial-canvas",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "pictorial-art-display-proof-qa-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      "# MNT-SITE-VIS-021A Pictorial Art Display Hidden Route QA",
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Pictorial Art Display direction and product request paths.",
