...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/hi-end-audio";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/hi-end-audio-hidden-route-20260523",
);
...
async function captureViewport(browser, viewport) {
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/audio",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/hi-end-audio",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "hi-end-audio-proof-qa-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      "# MNT-SITE-VIS-021A Hi-end Audio Hidden Route QA",
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Hi-end Audio direction and request paths.",
