...
const route = process.env.MONTELAR_QA_ROUTE || "/ru/creative-pack-proof/hologram-vitrine";
...
const artifactRoot = path.resolve(
  ...
  process.env.MONTELAR_QA_ARTIFACT_DIR ||
    "docs/strategy/artifacts/visual-qa/MNT-SITE-VIS-021A/hologram-vitrine-hidden-route-20260523",
);
...
async function captureViewport(browser, viewport) {
  ...
  return {
    ...
    primaryHref,
    secondaryHref,
    primaryPathOk: normalizePathname(primaryHref || "") === "/ru/hologram",
    secondaryPathOk: normalizePathname(secondaryHref || "") === "/ru/request/hologram-vitrine",
    screenshots: [
...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "hologram-vitrine-proof-qa-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(artifactRoot, "HUMAN_QA_NOTES.md"),
    [
      "# MNT-SITE-VIS-021A Hologram Vitrine Hidden Route QA",
      "",
      `Route: \`${route}\``,
      `Base URL: \`${baseUrl}\``,
      "",
      ...
      "- CTA hrefs checked for the real Hologram direction and request paths.",
